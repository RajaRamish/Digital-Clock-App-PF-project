...
int main() {
    ...
    amPmText.setPosition(digitalClock.getPosition().x + digitalClock.getGlobalBounds().width + 10, 220); // Positioned right next to time
    ...
    clockFace.setOutlineThickness(2.5); // Thin border (adjust thickness as needed)
    ...
    std::string labels[3] = { "Alarm", "Timer", "Settings" };
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                ...
                for (int i = 0; i < 3; ++i) {
                    if (buttons[i].getGlobalBounds().contains(static_cast<sf::Vector2f>(mousePos))) {
                        ...
                        if (labels[i] == "Alarm") {
                            addReminder();  // Add reminder functionality when "Alarm" is clicked
                        }
                        if (labels[i] == "Timer") {
